...
export default function FordDetail() {
  ...
  const fetchData = useCallback(
    async (opts = {}, showLoader = true) => {
      ...
      try {
        const res = await getCarsByBrand("Ford", { ...query, ...opts });
        setCars(res.data);
        if (allCars.length === 0) setAllCars(res.data);
      } catch (err) {
        console.error("Error fetching Ford cars:", err);
      } finally {
  ...
  return (
    <div className="newcars-page">
      ...
      <div className="page-header">
        <h1>Ford Cars</h1>
        <p className="breadcrumb">Homepage - Ford Cars</p>
      </div>
